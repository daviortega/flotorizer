...
Wallet = (function () {
	...
	Wallet.prototype.addAddress = function (address, data) {
		if (address in this.addresses) {
			console.log("Warning: address " + address + " already exists, skipping.");
		}
		else {
			...
			if (data.error !== false) {
				...
				try {
					...
					try {
						...
						console.log("There was an error rendering this page. Please contact an administrator.");
						console.log(ex);
					}
				}
				catch (ex) {
					console.log("Error decrypting wallet - Invalid password?");
					console.log(ex);
				}
			}
		}, "json").fail(function () {
			console.log("Error loading wallet from server. Possible connection problems");
		});
	};
	Wallet.prototype.store = function () {
		...
		$.post(flovaultBaseURL + "/wallet/update", {
			...
			if (data.error !== false) {
				console.log(data.error.message);
				console.log('WARNING: There was an error saving your wallet. ' +
	...
	Wallet.prototype.calculateBestUnspent = function (amount, unspents) {
		...
		if (CurrentAmount < amount) {
			...
			console.log("Not enough coins in unspents to reach target amount");
		}
		...
		if (this.validateKey(toAddress) && this.validateKey(fromAddress)) {
			if (fromAddress in this.addresses && this.validateKey(this.addresses[fromAddress].priv, true)) {
				...
				if (this.balances[fromAddress] < amount) {
					console.log("You don't have enough coins to do that");
					return;
				}
				this.getUnspent(fromAddress, function (data) {
					...
					if (amount < minFeePerKb) {
						console.log("You must send at least 0.001 FLO (otherwise your transaction may get rejected)");
						return;
					}
					...
					if ((amount + estimatedFee) > totalUnspent) {
						console.log("Can't fit fee of " + estimatedFee / Math.pow(10, 8) + " - lower your sending amount");
				...
				console.log("Error: You don't own that address!");
			}
		}
		else {
			console.log('Error: Your sending or recipient address is invalid. Please check for any typos');
		}
	};
...
/*$('#login-btn').click(function () {
	...
	$.get('/wallet/checkload/' + identifier, function (data) {
		if (data.error) {
			console.log('error loading wallet: ' + data.error.message);
		}
		else {
			console.log(data);
